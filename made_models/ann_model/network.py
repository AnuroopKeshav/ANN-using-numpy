...
class Model:
    ...
    def sigmoidDiff(z):
        return np.multiply((1.0 / (1.0 + np.exp(-z))), (1.0 - (1.0 / (1.0 + np.exp(-z)))))
    
    def tanhDiff(z):
        return 1.0 / (np.multiply((np.cosh(z)), np.cosh(z)))
        
    def leakyreLuDiff(z):
        return np.where(z > 0, 1.0, 0.1)

    def softmaxDiff(z):
        return np.multiply((np.exp(z) / sum(np.exp(z))), (1 - (np.exp(z) / sum(np.exp(z)))))
    ...
    # Dictionary referencing to the derivatives of the activation functions
    activationFunctions = {
        'relu': reLuDiff,
        'sigmoid': sigmoidDiff,
        'tanh': tanhDiff,
        'leakyrelu': leakyreLuDiff,
        'softmax': softmaxDiff
    }
        ...
        for epoch in range(self.epochs):
            ...
            for X, y in zip(X_train, y_train):
                ...
                for i in range((self.layer_count - 1), 0, -1):
                    dz = np.multiply(np.dot(self.W[i].T, self.DZ[-1]), self.activationFunctions[self.FN[i]](self.Z[i - 1]))
                    self.DZ.append(dz)
